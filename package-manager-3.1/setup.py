...
class Build(build):
    def run(self):
        ...
        # Copy slider
        print "Copying Pm-slider..."
        os.system("cp -R pm-slider/pm-slider /tmp")
        
        # find user name because setup.py super user
        user = os.listdir("/home/")
        if user[0] != "samba":
	    os.system("chown -R %s:users /tmp/pm-slider" % user[0])
	    print "User:%s" % user[0]
	else:
	    os.system("chown -R %s:users /tmp/pm-slider" % user[1])
	    print "User:%s" % user[1]
	    
        # Copy replacement slider
        os.system("cp -R pm-slider /usr/share/kde4/apps/package-manager")
