...
# buraya dikkatt
        ...
        for repo in repodb.list_repos():
            ...
            for package in doc.tags("Package"):
                if package.getTagData("Icon"):
                    for node in package.tags("Icon"):
                        if node.firstChild().data() == "0ad":
    ...
    def get_isa_packages(self, isa):
	    ...
	    for randomNumbers in range(0,14):
    ...
    def openSlider(self):
        ...
        for value in valueList:
            if os.path.isfile("%s%s.png" % (self.__tmpSliderDir, value)) == False:
                os.system("cp -R %s %s" % (self.__usrDir, self.__tmpDir))
                self.webView.setUrl(QtCore.QUrl(self.__tmpUrl))
                print "images not found for Pm-slider.\nCopying default file."
